...
|---|---|---|---|---|---|
| `UC-01` | Daniel · Chair 1 | Patient requests coffee | Support preference and current stable treatment status | Mira dispatches Atlas to complete a pre-approved support task and logs closure | No RN interruption; delivery remains visible in the event log |
| `UC-02` | Noah · Chair 2 | Patient reports anxiety and asks to end treatment early | Prescription, elapsed time, communication preference, and one prior shortened treatment | Mira preserves the exact request and assembles a concise decision card | Jordan decides; treatment state cannot change before the RN action |
